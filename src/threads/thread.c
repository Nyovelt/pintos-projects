                    ...
                    thread_func *function, void *aux)
{
  ...
  if (thread_mlfqs)
  {
    struct thread *_t = thread_current();
    t->nice = _t->nice;
    t->recent_cpu = _t->recent_cpu;
  }
...
init_thread(struct thread *t, const char *name, int priority)
{
  ...
  list_init(&t->locks);                      // p1.2
  t->wait_lock = NULL;                       // p1.2
...
next_thread_to_run(void)
{
  ...
  {
    struct list_elem *max_thread = list_min(&ready_list, compare_thread_priority, NULL);
...
bool compare_thread_priority(
    ...
    void *aux UNUSED)
{
  int p_a = list_entry(a, struct thread, elem)->priority;
  int p_b = list_entry(b, struct thread, elem)->priority;
  return p_a > p_b;
}

/* p1.2 use for update priority by donations and base_priority */
void priority_update(void)
{
  if (thread_mlfqs)
    return;
  int max_priority_among_all = thread_current()->base_priority;
  struct list_elem *e;
  struct list *locks = &thread_current()->locks;
  for (e = list_begin(locks); e != list_end(locks); e = list_next(e))
  {
    struct lock *lock = list_entry(e, struct lock, elem);
    if (max_priority_among_all < lock->max_priority)
    {
      max_priority_among_all = lock->max_priority;
    }
  }
  thread_current()->priority = max_priority_among_all;
}
...
void update_load_avg_and_recent_cpu()
{
  ...
  struct thread *t;
  for (struct list_elem *elem = list_begin(&all_list); elem != list_end(&all_list); elem = list_next(elem))
  {
